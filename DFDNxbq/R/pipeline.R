# ============================================================
# pipeline.R — DFDNxbq 管道函数
# ============================================================

#' Load data from NPZ + CSV (via reticulate)
#'
#' Uses reticulate to load a NumPy NPZ file containing signal arrays
#' and merges with metadata from a CSV file.
#'
#' @param npz_path Path to .npz file (keys: "acc_all", "gyr_all")
#' @param csv_path Path to .csv file (columns: SubjectID, ActivityID, TrialNo, IsFall)
#' @return List of data items, each with Acc_raw, Gyr_raw, IsFall, SubjectID, ActivityID, TrialNo
#' @export
...
#' Validate data quality before experiment
#'
#' Checks sample counts, class balance, subject diversity, and
#' fast_subjects intersection. Returns FALSE if data is severely
#' insufficient.
#'
#' @param data_list List of data items
#' @param labels Logical vector of fall labels
#' @param subjects Integer vector of subject IDs
#' @param fast_subjects Integer vector of test subject IDs
...
#' Run the full DFDNxbq pipeline
#'
#' High-level entry point that integrates data validation, stratified
#' sampling, denoising, and experiment execution.
#'
#' @param data_list List of data items (from \code{\link{load_data_from_npz}})
#' @param output_dir Output directory
#' @param sample_ratio Stratified sampling ratio (NULL = no sampling)
#' @param config_overrides List of configuration overrides (optional)
#' @return Result list from \code{\link{run_single_experiment}}
#' @export
